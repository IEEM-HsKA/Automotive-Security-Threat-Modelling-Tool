...
Collapsible::Collapsible(QWidget *parent)
    : QFrame(parent), m_mainLayout(new QVBoxLayout(this)),
      m_displayWidget(nullptr), m_titleButton(new QPushButton(this)),
      m_title(QString()) {
  setFrameShape(QFrame::NoFrame);
  setMaximumWidth(s_maxWidth);
  setStyleSheet(
      "Collapsible { border: 1px solid #808080; border-radius: 2px; }");
...
void Collapsible::updateTitle(const QString &title) {
  ...
  if (!title.isEmpty()) {
    m_title =
        fontMetrics.elidedText(title, Qt::ElideMiddle, s_maxWidth - arrowWidth);
  }

  // display the correct arrow
  // ... or if there is no display widget yet, always the up arrow, because
  // display widgets are opened by default
