...
PropertyEditor::PropertyEditor(QWidget *parent)
    : QScrollArea(parent), m_mainCtrl(nullptr) {
...
void PropertyEditor::showNodes(const QList<zodiac::NodeHandle> &selection) {
  ...
  for (QHash<zodiac::NodeHandle, Collapsible *>::iterator i = m_nodes.begin();
       i != m_nodes.end(); ++i) {
  ...
  for (zodiac::NodeHandle node : selection) {
    if (!m_nodes.contains(node)) {
      ...
      collapsible->setWidget(
          new NodeProperties(m_mainCtrl->getCtrlForHandle(node), collapsible));
      m_layout->insertWidget(
          0, collapsible); // insert the new Collapsible at the top
