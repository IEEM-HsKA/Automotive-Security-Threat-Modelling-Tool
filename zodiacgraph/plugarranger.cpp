...
int getProblemRows(QVector<int> &problemRows, const QVector<int> &guess,
                   const int rowCount) {
  int count = 0;
...
QVector<int> arrangePlugs(const QVector<qreal> &costTable, const int rowCount,
    ...
    for (int rowIndex = 0; rowIndex < problemRowCount; ++rowIndex) {
      ...
      for (int colIndex = 0; colIndex < emptyColumnCount; ++colIndex) {
        ...
        deltaTable[(emptyColumnCount * rowIndex) + colIndex] =
            costTable[(columnCount * row) + col] -
            costTable[(columnCount * row) + guess[row]];
      }
    }
  ...
  for (int leftIndex = 0; leftIndex < rowCount; ++leftIndex) {
    for (int rightIndex = leftIndex + 1; rightIndex < rowCount; ++rightIndex) {
      qreal currentSum =
          costTable[(columnCount * leftIndex) + guess[leftIndex]] +
