...
PlugLabel::PlugLabel(Plug *parent)
    : QGraphicsItem(parent), m_plug(parent), m_isHighlighted(false) {
...
void PlugLabel::updateShape() {
  ...
  m_text.prepare(QTransform(),
                 s_font); // prepare once to get the dimensions of the text
  ...
  m_text.prepare(
      m_transform,
      s_font); // prepare a second time with the correct transform matrix

  // update the bounding rect
  m_boundingRect =
      m_transform.mapRect(QRectF(0, 0, textSize.width(), textSize.height()));
}
...
QRectF PlugLabel::boundingRect() const { return m_boundingRect; }

void PlugLabel::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget * /*widget*/) {
