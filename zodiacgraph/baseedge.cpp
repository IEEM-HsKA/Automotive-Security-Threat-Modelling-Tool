...
BaseEdge::BaseEdge(Scene *scene)
    : QGraphicsObject(nullptr), m_scene(scene), m_arrow(nullptr),
      m_path(QPainterPath()), m_secondaryOpacity(0.), m_label(nullptr) {
...
BaseEdge::~BaseEdge() { setLabelText(""); }
...
QRectF BaseEdge::boundingRect() const {
  ...
  return m_path.boundingRect().marginsAdded(
      QMarginsF(overdraw, overdraw, overdraw, overdraw));
}

void BaseEdge::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                     QWidget * /* widget */) {
