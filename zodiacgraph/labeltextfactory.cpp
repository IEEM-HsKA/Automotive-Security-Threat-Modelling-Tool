...
LabelTextFactory::LabelTextFactory(const QSet<PlugEdge *> &edges)
  ...
  for (const PlugEdge *edge : edges) {
    ...
    QString fromName = startPlug->getNode()->getDisplayName() + s_dotChar +
                       startPlug->getName();
    QString toName =
        endPlug->getNode()->getDisplayName() + s_dotChar + endPlug->getName();
    m_maxNameLength =
...
QString LabelTextFactory::getHorizontalLine(int maxNameLength) {
  ...
  ///
  /// Also, to ensure that it is centered, the number of characters in the
  /// horizontal line should be odd. Therefore, the next smaller, odd number
  /// with regards to the longest line is the base for the calculation. It may
  /// however be visually pleasing to remove more characters from both sides of
  /// the line. This is the number of characters removed from both sides after
  /// the next smaller, odd number was found.
  ///
  ...
  int repeats = qMin(MAX_LENGTH, lineLength - ((lineLength % 2) + 1 +
                                               (UNDERLINE_SHORTAGE * 2)));
