...
static zodiac::Plug *findClosestPlug(const QPointF &pos,
                                     const QSet<zodiac::Plug *> &plugs,
                                     zodiac::Plug *closest);
...
Node::Node(Scene *scene, const QString &displayName, const QUuid &uuid)
    : QGraphicsObject(nullptr), m_scene(scene), m_displayName(displayName),
...
Plug *Node::getClosestPlugTo(const QPointF &pos, PlugDirection direction) {
  // to prevent mid-animation selection of plugs, narrow the allowed direction
  // if required
  if (direction == PlugDirection::BOTH) {
...
qreal Node::getPerimeterRadius() const { return m_perimeter->getRadius(); }
...
void Node::updateStyle() {
  ...
  m_perimeter->setOpacity(
      qMax(m_outgoingExpansionFactor, m_incomingExpansionFactor) *
...
QString Node::getUniquePlugName(const QString &name, const Plug *plug) {
  ...
  do {
    ...
    if (testPlug == plug) { // if the user wants to rename x_2 to x while x
                            // already exists, that's a no-op
      return uniqueName;
    }
  } while (testPlug);
  return uniqueName;
...
void Node::hasCompletelyCollapsed() {
  // this slot is called when the incoming OR outgoing plugs have collapsed but
  // we want only the second call
...
void Node::aboutToExpandAfresh() { arrangePlugs(); }
...
void Node::setExpansion(NodeExpansion newState) {
  ...
  if (newState == NodeExpansion::NONE) {
    ...
    fireAnimation(m_incomingCollapseAnimation, 0.0, m_incomingExpansionFactor,
                  s_collapseDuration, s_collapseCurve);
    fireAnimation(m_outgoingCollapseAnimation, 0.0, m_outgoingExpansionFactor,
                  s_collapseDuration, s_collapseCurve);
  } else {
    ...
    if (newState == NodeExpansion::IN) {
      ...
      fireAnimation(m_incomingExpandAnimation, 1.0, m_incomingExpansionFactor,
                    s_expandDuration, s_expandCurve);
      fireAnimation(m_outgoingCollapseAnimation, 0.0, m_outgoingExpansionFactor,
                    s_collapseDuration, s_collapseCurve);
      ...
      fireAnimation(m_incomingCollapseAnimation, 0.0, m_incomingExpansionFactor,
                    s_collapseDuration, s_collapseCurve);
      fireAnimation(m_outgoingExpandAnimation, 1.0, m_outgoingExpansionFactor,
                    s_expandDuration, s_expandCurve);
...
void Node::arrangePlugs() {
  ...
  // the deadzone is calculated at the height of the label at the inner radius
  // of an incoming plug it is the angular part of the perimeter that could be
  // taken up by the node's label
  ...
  QVector<qreal> zoneDirections(evenZoneCount);
  {
    ...
    qreal zoneSpan =
        (M_PI - (2. * halfDeadAngle) - ((halfZoneCount + 1) * gapAngle)) /
        halfZoneCount;
  ...
  for (Plug *plug : plugs) {
    ...
    // calling atan2 with (-y, x) turns the direction from Qt's screen
    // coordinates (with an inverted y-axis) to the one used by me, where
    // positive x is right, positive y is up and the zero-angle is on positive
    // x.
    QVector2D plugTarget = plug->getTargetNormal();
    plugDirections.append(
        QPair<Plug *, qreal>(plug, qAtan2(-plugTarget.y(), plugTarget.x())));
  }
  ...
  if (connectedPlugCount > 0) {
    ...
    for (int row = 0; row < connectedPlugCount; ++row) {
      ...
      for (int column = 0; column < evenZoneCount; ++column) {
        qreal cost = angularDistance(plugDirection, zoneDirections[column]) *
    ...
    int i = 0;
    for (int zoneIndex :
         zodiac::arrangePlugs(costTable, connectedPlugCount, evenZoneCount)) {
  ...
  if (plugCount < evenZoneCount) {
    ...
    // collapse the empty zone by extending the span of the other zones in its
    // half
    qreal newZoneSpan =
        (M_PI - (2. * halfDeadAngle) - ((halfZoneCount)*gapAngle)) /
        (halfZoneCount - 1);
  ...
  for (int plugIndex = 0; plugIndex < plugCount; ++plugIndex) {
    ...
    plugs.at(plugIndex)->defineShape(QVector2D(qCos(angle), -qSin(angle)),
                                     plugSweepAngle);
  }
}
...
qreal Node::getGapAngle() const { return s_plugGap / m_perimeter->getRadius(); }

qreal Node::getDeadZoneAngle() const {
  return m_label->getHeight() /
         (m_perimeter->getRadius() - (Plug::getWidth() * 1.5));
}
...
void Node::adjustRadius() {
  ...
  if (plugCount == 0) {
    m_perimeter->setRadius(
        0.); // resets the perimeter's radius to its minimal radius
    return;
  }
  ...
  qreal requiredArcLength = (s_plugSweep * plugCount) +
                            (s_plugGap * (plugCount + 2)) + (deadArcLength * 2);
  qreal radius = requiredArcLength / (2 * M_PI);
  m_perimeter->setRadius(
      radius); // perimeter does not shrink pass its minimal radius
...
///
/// \brief Sets up and fires an animation for a property animation targeting a
/// qreal.
///
...
///
/// If the plug is an incoming one, it is only considered, if it doesn not have
/// any edges connected yet.
///
...
static zodiac::Plug *findClosestPlug(const QPointF &pos,
  ...
  if (closest) {
    minDistSq =
        QVector2D((closest->getNormal()).toPointF() - pos).lengthSquared();
  } else {
    minDistSq = DBL_MAX;
  }
  ...
  for (zodiac::Plug *plug : plugs) {
    ...
    if ((plug->getDirection() == zodiac::PlugDirection::IN) &&
        (plug->getEdgeCount() > 0)) {
      continue;
    }
...
///
/// \brief Calculates the angular distance between two angles, always returning
/// a positive result.
///
