...
Perimeter::Perimeter(Node *parent)
    : QGraphicsObject(parent), m_node(parent), m_radius(s_minRadius) {
  // the perimeter needs to stack behind the parent
  setFlag(ItemStacksBehindParent);
  setCacheMode(DeviceCoordinateCache);

  // doesn't do anything with them, but needs to be enabled so they get send up
  // to the node
...
QRectF Perimeter::boundingRect() const { return quadrat(m_radius); }

void Perimeter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget * /* widget */) {
...
void Perimeter::hoverMoveEvent(QGraphicsSceneHoverEvent *event) {
  Plug *closestPlug =
      m_node->getClosestPlugTo(event->pos(), PlugDirection::BOTH);
...
void Perimeter::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event) {
  ...
  // calling the superclass is mandatory here, to treat doubleclick like normal
  // click PLUS something else. othwerise the draw edge does not get aquired
  // properly
