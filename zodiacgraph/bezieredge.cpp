...
BezierEdge::BezierEdge(Scene *scene)
    : BaseEdge(scene), m_startPoint(QPointF()), m_ctrlPoint1(QPointF()),
...
void BezierEdge::placeArrowAt(qreal fraction) {
  /// Technically, you cannot place the arrow all the way at the end of the edge
  /// because at that point, the spline would not have a tangent to orient the
  /// arrow to. We could of course say that if <code>fraction
  /// >= 1.0-VERY_SMALL_DELTA</code>, the direction would have to be calculated
  /// using a point before the position ... but why bother? It doesn't make a
  /// visual difference and would only introduce unnecessary code branching.
  ...
  m_arrow->setTransformation(edgeCenter,
                             std::atan2(edgeDirection.y(), edgeDirection.x()));
}
...
QPointF BezierEdge::getCtrlPointFor(Plug *plug) {
  ...
  case PlugDirection::BOTH:
    factor = qMax(plug->getNode()->getIncomingExpansionFactor(),
