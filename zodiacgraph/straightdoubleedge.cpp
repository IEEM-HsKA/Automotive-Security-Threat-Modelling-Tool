...
StraightDoubleEdge::StraightDoubleEdge(Scene *scene, EdgeGroupInterface *group,
  ...
  // the StraightEdge Constructor does so as well, but at that time this part of
  // the instance is not constructed yet so the wrong function is called (it
  // calls StraightEdge::updateShape instead).
  updateShape();
}
