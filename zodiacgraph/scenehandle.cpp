...
SceneHandle::SceneHandle(Scene *scene)
    : QObject(nullptr), m_scene(scene), m_isValid(scene != nullptr) {
  connectSignals();
}
...
NodeHandle SceneHandle::createNode(const QString &name, QString nodeType,
                                   const QUuid &uuid) {
#ifdef QT_DEBUG
  Q_ASSERT(m_isValid);
#else
...
#endif
  return NodeHandle(m_scene->createNode(name, nodeType, uuid));
}
...
void SceneHandle::sceneWasDestroyed() { m_isValid = false; }

void SceneHandle::printGraph() { m_scene->printBGLGraph(); }

Scene *SceneHandle::getScene() { return m_scene; }
