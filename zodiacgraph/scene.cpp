...
Scene::Scene(QObject *parent)
    : QGraphicsScene(parent), m_drawEdge(nullptr), m_nodes(QSet<Node *>()),
...
Node *Scene::createNode(const QString &name, const QUuid &uuid) {
  ...
  QUuid nodeID = newNode->getUniqueId();

  m_gManager.addNode(m_nodes.size(), nodeID.toString().toStdString(),
                     name.toStdString());

  return newNode;
}

Node *Scene::createNode(const QString &name, QString type, const QUuid &uuid) {
  Node *newNode = new Node(this, name, uuid);
  m_nodes.insert(newNode);
  addItem(newNode);
...
bool Scene::removeNode(Node *node) {
  ...
  m_gManager.deleteNode(nodeID.toString().toStdString());
...
PlugEdge *Scene::createEdge(Plug *fromPlug, Plug *toPlug) {
  //-----------------------------------//
  bool ok;
  QStringList list = EdgeDialog::getStrings(&ok);
  QString edgeName = "";
  QString edgeType = "";
  ...
  if (ok) {
    // santize edgeName
    edgeName = list[0];
    edgeName = edgeName.simplified();

    edgeType = list[1];
    edgeType = edgeType.simplified();

    qDebug() << edgeName << edgeType;
    // regular expression to find only the connection name
    QRegularExpression re(".\\w+ ");
    QRegularExpressionMatch match = re.match(edgeName);

    if (match.hasMatch()) {
      edgeName = match.captured(0);
      edgeName.replace(".", "");
    }
  } else {
    return nullptr;
  ...
  QString plugName = edgeName + "(" + edgeType + ")";

  fromNode->renamePlug(fromPlug, plugName);
  toNode->renamePlug(toPlug, plugName);
  ...
  if ((fromPlug->getDirection() != PlugDirection::OUT) ||
      (toPlug->getDirection() != PlugDirection::IN)) {
    return nullptr;
  }
  ...
  std::string srcNode = fromNode->getUniqueId().toString().toStdString();
  std::string destNode = toNode->getUniqueId().toString().toStdString();

  m_gManager.addEdge(srcNode, destNode, edgeName.toStdString(),
                     edgeType.toStdString());
...
void Scene::removeEdge(PlugEdge *edge) {
  ...
  // if the group is now empty, we can only delete it if the other group in the
  // pair is also empty
  ...
  // lastly, remove the QGraphicsItem from the scene, thereby taking possession
  // of the last pointer to the edge
  removeItem(edge);
...
void Scene::printBGLGraph() { m_gManager.printGraph(); }

GraphManager *Scene::getBGLgraph() { return &m_gManager; }
