...
View::View(QWidget *parent) : QGraphicsView(parent), m_zoomFactor(1.0) {
...
void View::mousePressEvent(QMouseEvent *event) {
  ...
  if (event->button() == s_dragMoveButton) {
    ...
    // do not allow rubberband selection with any button other than the
    // selection button
    setDragMode(QGraphicsView::NoDrag);
  }
...
void View::wheelEvent(QWheelEvent *event) {
  ...
  // do not call QGraphicsView::wheelEvent here, because it will scroll up or
  // down as well as zoom
  return;
}
...
void View::setScene(Scene *scene) { QGraphicsView::setScene(scene); }
