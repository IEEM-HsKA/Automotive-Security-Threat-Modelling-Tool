...
void NodeLabel::updateStyle() {
  ...
  // update the label
  QSizeF labelSize = QSizeF(qMax(textSize.width(), Node::getCoreRadius() * 2.),
                            textSize.height());
  m_outlineRect = QRectF((labelSize.width() / -2.) - s_horizontalMargin,
  ...
  m_boundingRect = m_outlineRect.marginsAdded(
      QMarginsF(halfLine, halfLine, halfLine, halfLine));

  update();
}

QRectF NodeLabel::boundingRect() const { return m_boundingRect; }

void NodeLabel::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget * /* widget */) {
  ...
  painter->drawRoundedRect(m_outlineRect, s_roundEdgeRadius, s_roundEdgeRadius,
                           Qt::AbsoluteSize);
...
void NodeLabel::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if ((p.x() * p.x()) + (p.y() * p.y()) <=
    ...
    // if the event is label was clicked inside the core, let the parent handle
    // it
    event->ignore();

  } else {
    // otherwise adjust the current selection "on-foot", since we cannot fall
    // back on Qt's own selection mechanism
