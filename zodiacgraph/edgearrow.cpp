...
QPolygonF EdgeArrow::s_originalArrow =
    QPolygonF() << QPointF(s_arrowHalfLength, 0.)
                << QPointF(-s_arrowHalfLength, -s_arrowHalfWidth)
                << QPointF(-s_arrowHalfLength, s_arrowHalfWidth);
QPolygonF EdgeArrow::s_originalDoubleArrow =
    QPolygonF() << QPointF(s_arrowHalfLength, s_doubleGap)
...
EdgeArrow::EdgeArrow(BaseEdge *edge)
    : QGraphicsObject(edge), m_edge(edge), m_arrowPolygon(QPolygonF()),
      m_kind(ArrowKind::SINGLE), m_label(nullptr) {
  // doesn't do anything with it but needs this flag in order to pass the event
  // on to the edge
...
void EdgeArrow::setTransformation(const QPointF &pos, qreal angle) {
  ...
  // force an update here, otherwise the arrow seems to "swim" on top of the
  // edge
  update();
}
...
QRectF EdgeArrow::boundingRect() const { return m_arrowPolygon.boundingRect(); }

void EdgeArrow::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget * /* widget */) {
