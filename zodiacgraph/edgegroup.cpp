...
EdgeGroup::EdgeGroup(Scene *scene, Node *fromNode, Node *toNode,
                     EdgeGroupPair *pair)
    : QObject(nullptr), EdgeGroupInterface(), m_scene(scene),
      m_fromNode(fromNode), m_toNode(toNode), m_pair(pair),
      m_edges(QSet<PlugEdge *>()), m_straightEdge(nullptr),
      m_bentEdgesCount(0) {
...
EdgeGroup::~EdgeGroup() {
  // As EdgeGroups are always deleted before the rest of the QGraphicsView, this
  // also work on shutdown
  ...
  delete m_straightEdge; // valgrind seems to mind a call to deleteLater()
                         // here... I've read it's not real but this works also
...
void EdgeGroup::removeEdge(PlugEdge *edge) {
  ...
  // if the edge to be removed is the last one in the group, it might be
  // 'unbent' to avoid confusion, temporarily set the bent count to 1 before
  // decreasing it again
...
uint EdgeGroup::getHash() const { return getHashOf(m_fromNode, m_toNode); }

bool EdgeGroup::isVisible() const { return m_straightEdge->isVisible(); }
...
void EdgeGroup::updateStyle() { m_straightEdge->updateStyle(); }
