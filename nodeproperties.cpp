...
QString NodeProperties::s_defaultPlugName = "Interface";

NodeProperties::NodeProperties(NodeCtrl *node, Collapsible *parent)
    : QWidget(parent), m_node(node), m_nextPlugIsIncoming(true) {
  ...
  m_plugLayout->setContentsMargins(
      0, 8, 0, 0); // leave space between the plug list and the name
  m_plugLayout->setColumnStretch(
      1, 1); // so the add-plug button always stays on the far right
  ...
  m_plugLayout->addWidget(new QLabel("Interface", this), 0, 0, 1, 2,
                          Qt::AlignLeft);
...
void NodeProperties::addPlugRow(zodiac::PlugHandle plug) {
  ...
  directionButton->setStatusTip("Toggle the direction of the Plug from "
                                "'incoming' to 'outoing' and vice versa.");
  ...
  m_plugRows.insert(
      plug.getName(),
      new PlugRow(this, plug, plugNameEdit, directionButton, removalButton));
}
...
PlugRow::PlugRow(NodeProperties *editor, zodiac::PlugHandle plug,
    ...
    : QObject(editor), m_editor(editor), m_plug(plug), m_nameEdit(nameEdit),
      m_directionToggle(directionToggle), m_removalButton(removalButton) {
...
void PlugRow::renamePlug() {
  m_nameEdit->setText(
      m_editor->getNode()->renamePlug(m_plug.getName(), m_nameEdit->text()));
}
