...
NodeCtrl::NodeCtrl(MainCtrl *manager, zodiac::NodeHandle node)
    : QObject(manager), m_manager(manager), m_node(node),
      m_plugs(QHash<PlugHandle, QList<PlugHandle>>()) {
  // connect node signals
  connect(&m_node, SIGNAL(removalRequested()), this, SLOT(remove()));
  connect(&m_node,
          SIGNAL(inputConnected(zodiac::PlugHandle, zodiac::PlugHandle)), this,
          SLOT(inputConnected(zodiac::PlugHandle, zodiac::PlugHandle)));
  connect(&m_node,
          SIGNAL(outputConnected(zodiac::PlugHandle, zodiac::PlugHandle)), this,
          SLOT(outputConnected(zodiac::PlugHandle, zodiac::PlugHandle)));
  connect(&m_node,
          SIGNAL(inputDisconnected(zodiac::PlugHandle, zodiac::PlugHandle)),
          this,
          SLOT(inputDisconnected(zodiac::PlugHandle, zodiac::PlugHandle)));
  connect(&m_node,
          SIGNAL(outputDisconnected(zodiac::PlugHandle, zodiac::PlugHandle)),
          this,
          SLOT(outputDisconnected(zodiac::PlugHandle, zodiac::PlugHandle)));
  connect(
      &m_node,
      SIGNAL(requestRenaming(const QString &oldName, const QString &newName)),
      this,
      SLOT(requestRenaming(const QString &oldName, const QString &newName)));
}

void NodeCtrl::rename(const QString &name) { m_node.rename(name); }
...
void NodeCtrl::setSelected(bool isSelected) { m_node.setSelected(isSelected); }

bool NodeCtrl::remove() { return m_manager->deleteNode(this); }
...
void NodeCtrl::requestRenaming(const QString &oldName, const QString &newName) {
  renamePlug(oldName, newName);
}